...
let generateSingleProductShop = () => {
  if (basket.length !== 0) {
    return (shop1.innerHTML = basket.map((x) => {
      //only id and item are in basket so get data rom data.js using id
      ...
      return `
                 <div class="first">
                    ...
                    <div class="singleProductdetails">
                        ...
                        <h4>Price : <i class="fa fa-rupee">${
                          search.price
                        }</i><br><br></h4>
                        <p>${search.fulldes}</p><br><br>
                        <div class="lastpartflex">
                            <h4>Total Amount : <i class="fa fa-rupee">${
                              search.price * x.item
                            }</i></h4><br>
                            <div class="buttons">
                                <i class="bi bi-plus-lg" onclick = "increment(${
                                  x.id
                                })"></i>
                                <div id = ${x.id} class="qty">${x.item}</div>
                                <i class="bi bi-dash-lg" onclick = "decrement(${
                                  x.id
                                })"></i>
                            </div>
                        </div>
